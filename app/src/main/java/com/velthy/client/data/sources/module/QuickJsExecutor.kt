...
internal object QuickJsExecutor {
    ...
    /** Query keys whose values are never written to the log. */
    private val SENSITIVE_QUERY = listOf("token", "key", "sig", "secret", "auth", "password", "cookie")
    ...
    /**
     * A module URL with anything credential-shaped in its query string taken
     * out, so a fetch can be logged without writing a token into the log.
     */
    private fun redactModuleUrl(url: String): String {
        ...
        val redacted = url.substring(queryStart + 1).split('&').joinToString("&") { pair ->
            ...
            if (pair.contains('=') && SENSITIVE_QUERY.any { it in key }) "$key=…" else pair
        }
